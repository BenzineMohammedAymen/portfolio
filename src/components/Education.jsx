...
function EduCard({ edu, delay }) {
  ...
  return (
    <div
      ref={ref}
      className={`reveal ${inView ? "in-view" : ""} bg-card border border-border rounded-3xl p-8 relative overflow-hidden group hover:border-white/15 hover:-translate-y-1 transition-all`}
    ...
    >
      ...
      <div className="text-lg font-bold text-white mb-1">{edu.degree}</div>
      ...
      <p className="text-sm text-slate-400 leading-relaxed mb-4">{edu.desc}</p>
