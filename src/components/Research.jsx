  ...
  return (
    <section id="research" className="bg-bg2 py-24 px-6 md:px-10">
      <div className="max-w-6xl mx-auto">
        ...
        >
          ...
          <div className="flex flex-wrap gap-6 mb-6 relative z-10">
            {research.meta.map((m) => {
              ...
              return (
                <div key={m.text} className="flex items-center gap-1.5 text-sm text-muted">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            {research.findings.map((f) => (
              <div key={f.label} className="bg-card/60 border border-line/10 rounded-xl p-4 backdrop-blur">
                <div className="text-xl font-extrabold bg-gradient-to-br from-indigo to-violet bg-clip-text text-transparent">{f.num}</div>
                <div className="text-xs text-muted mt-1 leading-relaxed">{f.label}</div>
              </div>
            ))}
          </div>
        ...
        <h3 className="text-base font-bold text-fg mb-5">My Research Interests</h3>
          ...
          {researchInterests.map((ri) => {
            ...
            return (
              <div key={ri.name} className="bg-card border border-border rounded-2xl p-5 text-center hover:border-indigo hover:-translate-y-1 transition-all">
                ...
                <div className="text-sm font-bold text-fg">{ri.name}</div>
                <div className="text-xs text-muted mt-1">{ri.desc}</div>
              </div>
            );
          })}
        </div>
